...
int init_yolov8_model(const char *model_path, rknn_app_context_t *app_ctx) {
    ...
    printf("init_yolov8_model zero copy!!!\n");
    // Load RKNN Model
...
int inference_yolov8_model(rknn_app_context_t *app_ctx, image_buffer_t *img, object_detect_result_list *od_results) {
    printf("inference_yolov8_model rknpu2 zero copy\n");
    struct timeval start2, end2, start3, end3, start4, end4, start5, end5;
    int ret;
    ...
    // 计算耗时（微秒→毫秒）
    double time_use2 = (end2.tv_sec - start2.tv_sec) * 1000.0 +
    ...
    printf("NC1HWC2 to NCHW time use: %f ms\n", time_use4);
    // 开始计时
    ...
    printf("post process time use: %f ms\n", time_use5);
    for (int i = 0; i < app_ctx->io_num.n_output; i++) {
        free(outputs[i].buf);
    }
