...
static int convert_image_rga(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color)
{
    ...
    //printf("******RGA convert image success\n");
...
// 计算裁剪区域的简化版本
static image_rect_t getCropRect(int bigWidth, int bigHeight, 
                          ...
                          image_rect_t box) 
{
    ...
    // 1. 目标中心点
    int centerX = (box.right + box.left) / 2;
    ...
    if (right > bigWidth) {
        int offset = right - bigWidth;
        right = bigWidth;
        left = bigWidth - cropWidth;
    }
    
    if (bottom > bigHeight) {
        int offset = bottom - bigHeight;
        bottom = bigHeight;
        top = bigHeight - cropHeight;
    }
    
    // 5. 设置结果
    rect.left = left;
    rect.top = top;
    rect.right = right - 1;  // 转换为右下角坐标，包含表示法
    rect.bottom = bottom - 1;  //转换为右下角坐标，包含表示法
    
    return rect;
}
int crop_alg_image(image_buffer_t* src_image, image_buffer_t* crop_image, image_rect_t box, image_rect_t* crop_rect, int crop_width, int crop_height)
{
    int ret = 0;
    if (src_image == NULL || crop_image == NULL) {
        return -1;
    }

    crop_image->width = crop_width;
    crop_image->height = crop_height;
    crop_image->width_stride = crop_image->width;
    crop_image->height_stride = crop_image->height;
    crop_image->format = src_image->format;
    int crop_size = crop_width * crop_height * 3; // assume max 3 bytes per pixel
    if (crop_image->virt_addr == NULL && crop_image->fd <= 0) {
        crop_image->virt_addr = (uint8_t *)malloc(crop_size);
    ...
    *crop_rect = getCropRect(
        src_image->width, src_image->height,
        crop_image->width, crop_image->height,
        box
    );

    printf("~~~~~~crop rect: left=%d top=%d right=%d bottom=%d\n",
        crop_rect->left, crop_rect->top, crop_rect->right, crop_rect->bottom);

    image_rect_t src_box;
    src_box.left = crop_rect->left;
    src_box.top = crop_rect->top;
    src_box.right = crop_rect->right;
    src_box.bottom = crop_rect->bottom;

    image_rect_t dst_box;
    dst_box.left = 0;
    dst_box.top = 0;
    dst_box.right = crop_image->width - 1;
    dst_box.bottom = crop_image->height - 1;

    ret = convert_image(src_image, crop_image, &src_box, &dst_box, 0);
    return ret;
}